...
3. **錯誤選項分析卡片**（Top 3-5 題，精簡版）
   - 最常選錯的選項（選項層級分析）
   - 顯示每題最常被選的錯誤選項
   - 點擊可跳轉到該題

4. **需要加強的題目列表**（完整列表，保留原有 UI）
   - 依錯誤率排序的題目列表（最多 30 題）
   - 題目層級的弱點展示
   - 顯示錯誤率、答錯次數
   - 特殊標記（最需要搶救、次要警告等）
...
| Phase 2: API 開發 | ✅ 100% (8/8) | 2026-02-26 |
| Phase 3: 前端整合 | 33% (1/3) | 進行中 |
| Phase 4: 文件更新 | 0% | 待定 |
